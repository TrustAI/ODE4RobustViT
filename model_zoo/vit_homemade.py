...
class EncoderBlock_merger(nn.Sequential):
    def __init__(self,
                 em_size: int=512, 
                 forward_expansion: int = 4,
                 forward_drop_out: float = 0.,
...
class Encoder_merger(nn.Sequential):
    def __init__(self, depth: int = 12, **kwargs):
        super().__init__(*[EncoderBlock_merger(**kwargs) for _ in range(depth)])
...
class ClassificationHead_cls_token(nn.Sequential):
    def __init__(self, em_size: int = 512, n_classes: int = 10):
        super().__init__(
            TakeFirst(),
            nn.LayerNorm(em_size), 
            nn.Linear(em_size, n_classes))
...
class ViT_merger(nn.Sequential):
    def __init__(self,     
                in_channels: int = 3,
                patch_size: tuple = (16, 16),
                em_size: int = 512,
        ...
        super().__init__(
            Embedding(in_channels, img_size, patch_size, em_size),
            Encoder_merger(depth, em_size=em_size, **kwargs),
            ClassificationHead(em_size, n_classes)
        )
