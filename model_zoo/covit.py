...
class EncoderBlock_Conv1D_merger(nn.Sequential):
    def __init__(self,
                 em_size: int=512, 
                 forward_expansion: int = 4,
                 forward_drop_out: float = 0.,
...
class Encoder_merger(nn.Sequential):
    def __init__(self, depth: int = 4, **kwargs):
        super().__init__(*[EncoderBlock_Conv1D_merger(**kwargs) for _ in range(depth)])
...
class CoViT_merger(nn.Sequential):
    def __init__(self,     
                in_channels: int = 3,
                patch_size: tuple = (16, 16),
                em_size: int = 512,
...
if __name__ == '__main__': 
    x = torch.rand([10, 197, 512])
    print(x.shape)
    con1d = MultiHeadConv1D()
    con1d(x).shape
